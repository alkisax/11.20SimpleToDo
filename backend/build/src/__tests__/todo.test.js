...
const app_1 = __importDefault(require("../app")); // your Express app entrypoint
const todo_dao_1 = __importDefault(require("../dao/todo.dao")); //gpt
...
beforeEach(async () => {
    await mongoose_1.default.connection.collection('todo').deleteMany({});
});
...
describe('POST /api/todo', () => {
    ...
    it('should return 500 with unknown error when thrown error is not an instance of Error', async () => {
        ...
        jest.spyOn(todo_dao_1.default, 'create').mockImplementation(() => {
...
describe('GET /api/todo', () => {
    it('should return an empty array when no todos exist', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/api/todo');
        expect(res.status).toBe(200);
        expect(res.body.status).toBe(true);
    ...
    it('should return all todos that exist in the database', async () => {
        ...
        await Promise.all(todos.map(todo => (0, supertest_1.default)(app_1.default).post('/api/todo').send(todo)));
        const res = await (0, supertest_1.default)(app_1.default).get('/api/todo');
        expect(res.status).toBe(200);
        expect(res.body.status).toBe(true);
    ...
    it('should return 500 with unknown error when thrown error is not an instance of Error', async () => {
        ...
        jest.spyOn(todo_dao_1.default, 'readAll').mockImplementation(() => {
...
describe('GET /api/todo/:id', () => {
    it('should return 404 if todo with given id does not exist', async () => {
        ...
        const res = await (0, supertest_1.default)(app_1.default).get(`/api/todo/${fakeId}`);
    ...
    it('should return 200 and the todo data if todo exists', async () => {
        ...
        const res = await (0, supertest_1.default)(app_1.default).get(`/api/todo/${todoId}`);
        expect(res.status).toBe(200);
        expect(res.body.status).toBe(true);
        expect(res.body.data).toMatchObject(todoData);
    ...
    it('should return 500 on invalid id format', async () => {
        ...
        const res = await (0, supertest_1.default)(app_1.default).get(`/api/todo/${invalidId}`);
        expect(res.status).toBe(500);
        expect(res.body.status).toBe(false);
